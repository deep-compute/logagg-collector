...
class LogCollector:
    ...
    def _init_fpaths(self):
        ...
        """
        self.state["fpaths"] = [
            {"fpath": self.SERVERSTATS_FPATH, "formatter": self.DOCKER_FORMATTER}
        ]
        self.state.flush()
    ...
    def _init_nsq_sender(self):
        ...
        if get_topic_info_result["result"].get("success"):
            ...
            nsq_sender_heartbeat = NSQSender(
                nsqd_http_address, heartbeat_topic, self.log
            )
            nsq_sender_logs = NSQSender(nsqd_http_address, logs_topic, self.log)
    ...
    def _compute_md5_fpatterns(self, fpath):
        ...
        dir_contents = [
            ...
            if bool(self.LOGAGGFS_FPATH_PATTERN.match(f)) and isfile(join(d, f))
        ]
        ...
        for f in self.state["fpaths"]:
            ...
            for fpath in glob.glob(f["fpath"]):
                ...
                if fpattern == None:
                    continue
                self.log.debug(
                    "_scan_fpatterns", fpattern=fpattern, formatter=formatter
                )
                try:
                ...
                log_f = dict(
                    fpattern=fpattern, formatter=formatter, formatter_fn=formatter_fn
                )
...
class CollectorService:
    ...
    def _get_nsq(self):
        ...
        """
        return dict(
            nsqd_http_address=self.collector.nsq_sender_logs.nsqd_http_address,
